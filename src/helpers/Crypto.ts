const ArrayBufferToUrlSafeBase64 = {
    ...
    _charCodeToNumber(charCode: number) {
    ...
    encode(arrayBuffer: ArrayBuffer) {
...
export const Password = {
    ...
    _countChars(buffer: Uint8Array, start: number, end: number, maxSpecial: number) {
    ...
    _decode(buffer: Uint8Array, start: number, end: number) {
        let password = '';
...
export const CryptoTasks = {
    ...
    async encrypt(keyPayload: string, plaintext: string) {
    ...
    async decrypt(payload: string, password: string) {
